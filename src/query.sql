...
select count(plane) from trip where plane = 'TU-134';
--#6 Какие компании совершали перелеты на Boeing
select * from Trip join company on trip_id = company_id;--не сделал
--#7 Вывести все названия самолётов, на которых можно улететь в Москву (Moscow)
select plane, town_from from trip where town_from = 'Moscow';
--#9 Какие компании организуют перелеты с Владивостока (Vladivostok)?
select company_id,town_from,town_to from trip where (town_from,town_to) = 'Vladivostok';--ne sdelal
--#12 Вывести id и количество пассажиров для всех прошедших полётов
--
--#14 В какие города летал Bruce Willis
SELECT * FROM trip JOIN passenger ON plane = name;
--#16 Сколько рейсов совершили авиакомпании с Ростова (Rostov) в Москву (Moscow) ?
select count(trip_id)from Trip where town_from = 'Rostov' and town_to = 'Moscow';
...
select count(trip_id)from Trip where town_to = 'Moscow' and town_from = 'Rostov' ;
